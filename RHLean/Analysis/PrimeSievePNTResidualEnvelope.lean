...
/-- The deep residual envelope attached to one occupied the earlier development block.  It skips
exactly the same `min J j` shallow levels used by the the earlier development deep-energy split. -/
...
/-- **PNT localization in the exact the earlier development deep coordinate.**  For every target
