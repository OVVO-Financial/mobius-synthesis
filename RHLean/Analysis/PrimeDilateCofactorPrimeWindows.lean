...
/-- In one unresolved `q`-fibre, the the earlier development prime-dilate shell is exactly the
...
prime coordinate, every lower-scale Mertens value can be compressed by the earlier development and
...
the earlier development shell is strictly smaller than `R`. -/
