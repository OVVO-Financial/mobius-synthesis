...
An earlier layer isolates the remaining packet-side arithmetic input as the signed
...
existing energies block by block and identifies exactly which blocks need
...
of packet-tree energies already used in earlier layers. Then:
...
/-- On a live block, its released contribution is literally the signed
